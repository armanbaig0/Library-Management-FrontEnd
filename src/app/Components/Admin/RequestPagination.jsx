...
const RequestPagination = ({ bookReq, onRequestsUpdate }) => {
  ...
  const handleRequestAction = async (request_id, action) => {
    try {
      const response = await axios.post('http://localhost:5000/admin/handle-request', {
        request_id,
        action
      });
      ...
      if (response.data.success) {
        ...
        // Refresh requests
        if (onRequestsUpdate) {
          const updatedRequests = await axios.get('http://localhost:5000/admin/get-req');
          ...
          <thead>
            <tr>
              <td className='justify-center items-center w-[15%]'>
                <h3 className="text-md text-center font-bold mb-2">Student Name</h3>
              </td>
              <td className='justify-center items-center w-[15%]'>
                ...
                <thead>
                  <tr className="shadow-md flex">
                    <td className="w-[15%] px-4 py-2">
                      <h4 className="text-gray-800 text-center text-md">{book_request.student_name}</h4>
                    </td>
                    <td className="w-[15%] px-4 py-2">
                      <h4 className="text-gray-800 text-center text-md">{book_request.book_name}</h4>
                    </td>
                    <td className="w-[15%] px-4 py-2">
                      <h4 className="text-gray-800 text-center text-md">{book_request.book_author}</h4>
                    </td>
                    <td className="w-[55%] justify-end flex px-2 py-2">
                      {book_request.status === 'pending' ? (
                        <>
                          <button 
                            onClick={() => handleRequestAction(book_request.request_id, 'accept')}
                            className="bg-blue-500 hover:bg-blue-600 text-white py-1 px-3 rounded-md mr-2 cursor-pointer"
                          >
                            Accept
                          </button>
                          <button 
                            onClick={() => handleRequestAction(book_request.request_id, 'reject')}
                        ...
                        <span className={`text-md ${book_request.status === 'accepted' ? 'text-green-600' : 'text-red-600'}`}>
                          {book_request.status.charAt(0).toUpperCase() + book_request.status.slice(1)}
                        </span>
                      )}
                    </td>
                  </tr>
